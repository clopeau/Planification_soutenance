...
# --- CONFIGURATION ---
st.set_page_config(page_title="Planification Soutenances v14 (Export Excel)", layout="wide", page_icon="🎓")
...
# --- HELPERS & IMPORTERS (Identiques v13) ---
def clean_str(val):
...
def importer_disponibilites(uploaded_file, tuteurs_connus, co_jurys_connus, horaires_config):
    ...
    dispos_data = {}; treated = set(); logs = []; col_nom = df.columns[0]
    for _, row in df.iterrows():
...
class SchedulerEngine:
    ...
    def _solve_single_run(self):
        ...
        for _, etu in student_queue:
            tuteur = etu['Tuteur']; best_move = None; best_score = -float('inf')
